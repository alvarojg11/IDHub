  ...
  return (
    <section className="mx-auto max-w-6xl px-2 py-10 sm:px-4">
        ...
        <section className="mt-8">
          <div className="flex flex-wrap gap-2">
            {categories.map((category) => (
              <span
                key={category.key}
                className="border border-[var(--border)] bg-white px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]"
              >
          ...
          <article className="overflow-hidden border border-[var(--border)] bg-white">
            <div className="grid lg:grid-cols-[minmax(320px,0.95fr)_minmax(0,1.05fr)]">
              ...
              <div className="p-6 sm:p-8">
                <div className="flex flex-wrap gap-2">
                  <span className="border border-[var(--border)] bg-[var(--primary-tint)] px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--primary)]">
                    {featuredEntry.historicalDateLabel}
                  </span>
                  {featuredEntry.categories.map((category) => (
                    <span
                      key={category}
                      className="border border-[var(--border)] px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]"
                    >
      ...
      <section className="mt-10">
        ...
        <div className="idhub-panel p-5 sm:p-6">
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {entries.map((entry) => (
              <article
                key={entry.slug}
                className="group overflow-hidden border border-[var(--border)] bg-white transition hover:border-[var(--border-strong)] hover:bg-[var(--background-soft)]"
              >
